...
#include "tflite/experimental/litert/runtime/ahwb_buffer.h"
...
TEST(Buffer, GpuWriteAhwbRead) {
  ...
  float* ahwb_host_data_after_write =
      reinterpret_cast<float*>(ahwb_host_data_after_write_data);
  ASSERT_NE(ahwb_host_data_after_write, nullptr);
  // Check that the data is the same as the GPU write.
  for (int i = 0; i < 4; ++i) {
    EXPECT_THAT(ahwb_host_data_after_write[i],
                FloatEq(static_cast<float>(i) / 10.0));
  }
  LITERT_ASSERT_OK(AhwbBuffer::Unlock(ahwb_buffer.ahwb));
}
